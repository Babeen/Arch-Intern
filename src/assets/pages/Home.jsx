...
const Home = () => (
  <MainLayout transparentNav>
    ...
    <section className="relative h-screen flex items-center justify-center text-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-white">
        ...
        >
          <Link
            to="/products"
            className="group flex items-center gap-2 bg-white text-gray-900 font-semibold px-8 py-3.5 rounded-full hover:bg-amber-300 transition-all duration-300"
          >
            Shop Now
          ...
          <Link
            to="/collections"
            className="flex items-center gap-2 border border-white/60 text-white font-semibold px-8 py-3.5 rounded-full hover:bg-white/10 backdrop-blur-sm transition-all duration-300"
          >
            Explore Collections
          </Link>
        </motion.div>
    ...
    {/* ── Features Bar ── */}
    <div className="bg-gray-950 text-white border-b border-white/10 py-5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {featuredProducts.map((product, i) => (
            <motion.div
              key={product.id}
              className="group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500"
    ...
    <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <motion.div
        className="relative overflow-hidden rounded-3xl bg-gray-950 text-white px-8 md:px-16 py-16 text-center"
        variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
      >
        {/* Decorative blobs */}
        ...
        >
          <input
            ...
            className="flex-1 bg-white/10 border border-white/20 text-white placeholder-white/40 rounded-full px-5 py-3 text-sm focus:outline-none focus:border-amber-400 transition-colors"
          />
          <button
            type="submit"
            className="bg-amber-400 text-gray-900 font-semibold px-7 py-3 rounded-full hover:bg-amber-300 transition-colors whitespace-nowrap"
          >
            Subscribe
          </button>
        </form>
