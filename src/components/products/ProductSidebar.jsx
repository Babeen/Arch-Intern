    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-[0.2em] font-semibold text-gray-900 dark:text-white">Filters</span>
        </div>
        {hasActiveFilters && (
          <button onClick={clearAll} className="text-xs text-gray-400 hover:text-amber-500 transition-colors flex items-center gap-1">
      ...
      <div>
        <p className="text-xs uppercase tracking-[0.2em] font-semibold text-gray-400 dark:text-gray-500 mb-3">Search</p>
      ...
      <div>
        <p className="text-xs uppercase tracking-[0.2em] font-semibold text-gray-400 dark:text-gray-500 mb-3">Category</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs uppercase tracking-[0.2em] font-semibold text-gray-400 dark:text-gray-500">Max Price</p>
          <span className="text-sm font-bold text-amber-500">${priceRange}</span>
        </div>
        <input
          type="range" min={0} max={1000} step={10} value={priceRange}
          onChange={(e) => setPriceRange(Number(e.target.value))}
          className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-amber-400 bg-gray-200 dark:bg-white/10"
        />
  ...
  return (
    <>
      ...
      >
        ...
        Filters
        {hasActiveFilters && (
          <span className="bg-amber-400 text-gray-900 text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">!</span>
        )}
      </button>
